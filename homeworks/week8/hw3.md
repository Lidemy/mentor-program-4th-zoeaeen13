...
* Asynchronous：非同步
* JavaScript：使用的程式語言
* XML：原指回應的資料格式，近年由於 JSON 流行，使用 Ajax 處理的資料不限於 XML。以Web API 設計角度來看，與表現層無關的 JSON 格式比較乾淨，可以獲得比較好的重複使用性
...
3. 到底差在哪裡？`.onreadystatechange()`  V.S `.onload()`
...
拿註冊登錄來說，一般用表單提交會觸發 submit 事件，使頁面發生跳轉，但是這個跳轉的行為掌控權在於後端，而我們希望控制權放在前端時，可以用 JS 操作數據和頁面變化，或者說，某些時候我們還不想要跳轉（比如送出資料前就先驗證用戶輸入的帳號是否有效），但是總不可能在 submit 的時候才送查詢請求，這時候就需要用到 Ajax，進行異步請求。
